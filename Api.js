...
module.exports.drizzly = {
    ...
    users: async function apifunc2(req, res, next) {
        ...
        let data = {
            ...
            url: req.url,
            token: req.params.token ? { get: req.params.token, status: token.current == "true" ? "authorized" : "expired" } : "not written",
            data: token ? token.current == "true" ? users ? users : "error" : "expired" : "not written"
        }
        return res.json(data)
    }
}
module.exports.discord = async function apifunc3(req, res, next) {
...
module.exports.twitter = async function apifunc4(req, res, next) {
...
module.exports.weather = async function apifunc5(req, res, next) {
...
module.exports.error = async function apifunc6(req, res, next) {
    let data = {
        status: "OK",
        url: {
            url: req.url,
